<template>
	<view class="wx-content-info" v-show="closepreview">
		<!--  -->
		<view class='cropper-content' >
			<view v-if="isShowImg" class="wx-corpper" :style="{width:cropperInitW+'px',height:cropperInitH+'px'}" style="background:#FFFFFF">
				<view class="wx-corpper-content" :style="{width:cropperW+'px',height:cropperH+'px',left:cropperL+'px',top:cropperT+'px'}">
					...
					<view v-show="cropperBox" class="wx-corpper-crop-box" @touchstart="contentStartMove" @touchmove="contentMoveing" 
			...
			<button v-show="cropperBox" @tap="getImageInfo" style='margin-top: 30upx;width: 50%;'> 确定裁剪 </button>
		</view>
		...
		<canvas canvas-id="myCanvas" :rotateAngle="rotateAngle" :style="{ width:qualityWidth+'px',height:qualityWidth/innerAspectRadio+'px'}" style="position:absolute;border:1upx solid red;top:-9999upx;left:-9999upx;"></canvas>
	...
	export default {
		...
		methods: {
			...
			getImageInfo() {
				...
				// this.$emit("update", _this.imageSrc, canvasW,canvasH)
				...
				ctx.draw(true, () => {
					...
					debugger
					// 生成图片，把当前画布指定区域的内容导出生成指定大小的图片,把图片传给父组件写入父组件画布
					uni.canvasToTempFilePath({
						x: canvasL,
						y: canvasT,
						width: canvasW, //画布的宽度
						height: canvasH,
						destWidth: canvasW, //图片的宽度
						destHeight: canvasH,
						quality: 0.5,
						canvasId: 'myCanvas',
						success: function(res) {
							...
							// this.$emit("update", _this.imageSrc, canvasW,canvasH) //裁剪好的图片传给父组件
						},
						fail:function(errs){
							debugger
				...
				this.$emit("update", _this.imageSrc) //裁剪好的图片传给父组件
				debugger
			},
