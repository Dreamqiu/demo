<template>
	...
	<canvas canvas-id="canvas-drag" disable-scroll="true" @touchstart="start"
	@touchmove="move" @touchend="end" :style="'width: ' + width + 'px; height: ' + height + 'px;'"
	style="margin: 0 auto;border: 1px solid #717171;border-radius: 20rpx;">
...
const dragGraph = function ({
  ...
  if (type === 'text') {
	  if(textDirection === '横向'){
		 canvas.setFontSize(fontSize);  //设置字体的字号。
		 const textWidth = canvas.measureText(text).width; //测量文本尺寸信息，目前仅返回文本宽度
		 ...
		 this.h = textHeight; 
		  ...
		  canvas.setFontSize(fontSize);  //设置字体的字号。
		  const textWidth =  fontSize + 10;//测量文本尺寸信息，目前仅返回文本宽度
		  const textHeight = canvas.measureText(text).width;
		  this.centerX = x + textWidth / 2;
		  this.centerY = y + textHeight / 2;
		  this.w = textWidth;
		  this.h = textHeight;
  ...
  this.square = [[this.x, this.y], [this.x + this.w, this.y], [this.x + this.w, this.y + this.h], [this.x, this.y + this.h]]; // 4个顶点坐标
  this.fileUrl = url;
  this.text = text;
...
dragGraph.prototype = {
   ...
   */
  paint() {
    this.ctx.save(); // 由于measureText获取文字宽度依赖于样式，所以如果是文字元素需要先设置样式；save保存当前的绘图上下文。
    ...
    if (this.type === 'text') {
      ...
      this.ctx.setTextBaseline('middle'); //设置文字水平对齐方式可选值 'top'、'bottom'、'middle'、'normal'
      this.ctx.setTextAlign('center'); //用于设置文字的对齐,可选值 'left'、'center'、'right'
      this.ctx.setFillStyle(this.color); //设置文字填充色
	...
	// 旋转元素
    this.ctx.translate(this.centerX, this.centerY);   //对当前坐标系的原点(0, 0)进行变换，默认的坐标系原点为页面左上角。
    this.ctx.rotate(this.rotate * Math.PI / 180); //以原点为中心，原点可以用 translate方法修改。
    this.ctx.translate(-this.centerX, -this.centerY); 
	
	// 渲染元素
    if (this.type === 'text') {
		
		if(this.textDirection == '横向'){
			this.ctx.save();
			textWidth = this.ctx.measureText(this.text).width;
			textHeight = this.fontSize + 10; // 字体区域中心点不变，左上角位移	
			this.x = this.centerX - textWidth / 2;
			...
			this.ctx.fillText(this.text, this.centerX, this.centerY); //在画布上绘制被填充的文本。
			this.ctx.restore();	
		}else{
			textWidth = this.fontSize + 10;
			textHeight = this.ctx.measureText(this.text).width; 
			this.x = this.centerX - textWidth / 2;
			// 字体区域中心点不变，左上角位移
			// this.y = this.centerY - textHeight / 2;
			this.y = this.centerY;
			
			let letterSpacing = 0;  //字体间的距离
			for(let i = 0; i < this.text.length; i++){
			    const str = this.text.slice(i,i+1).toString();
			    if(str.match(/[A-Za-z0-9]/)&&(this.centerY<576)){ // 非汉字 旋转
			        this.ctx.save();
			        this.ctx.translate(this.centerX,this.centerY);
			        this.ctx.rotate(Math.PI/180*90);
			        this.ctx.textBaseline = 'bottom';
					this.ctx.fillText(str,0,10);
			        // this.ctx.fillText(str,this.centerX,this.centerX);
			       this.ctx.restore();
			       this.centerY+=this.ctx.measureText(str).width+letterSpacing; // 计算文字宽度
			    }else if(str.match(/[\u4E00-\u9FA5]/)&&(this.centerY<576)){
			        this.ctx.save();
			        this.ctx.textBaseline = 'top';
			        this.ctx.fillText(str,this.centerX,this.centerY);
			        this.ctx.restore();
			        this.centerY+=this.ctx.measureText(str).width+letterSpacing; // 计算文字宽度
			    }
			}
      ...
      this.ctx.drawImage(this.fileUrl, this.x, this.y, this.w, this.h);  //绘制图像到画布。
    }
	
	// 如果是选中状态，绘制选择虚线框，和缩放图标、删除图标
    if (this.selected) {
		this.ctx.save();
      this.ctx.setLineDash([2, 5]);  //设置线条宽度。
      this.ctx.setLineWidth(2); //设置线条的宽度。
      this.ctx.setStrokeStyle(STROKE_COLOR); //设置边框颜色，默认黑色
      this.ctx.lineDashOffset = 6;  //虚线偏移量，初始值为0
      if (this.type === 'text') {
		  
		  this.ctx.strokeRect(this.x, this.y, textWidth, textHeight);  //画一个矩形(非填充)。用 setStrokeStyle() 设置边框颜色，如果没设置默认是黑色。
   ...
   */
  _drawBorder() {
    let p = this.square;  //四个顶点的坐标this.square = [[this.x, this.y], [this.x + this.w, this.y], [this.x + this.w, this.y + this.h], [this.x, this.y + this.h]]; // 4个顶点坐标
    let ctx = this.ctx;
    this.ctx.save();
    this.ctx.beginPath(); //开始创建一个路径，需要调用fill或者stroke才会使用路径进行填充或描边。
    ctx.setStrokeStyle('orange'); //设置边框颜色
   ...
   */
  _draw_line(ctx, a, b) {
    ctx.moveTo(a[0], a[1]); //把路径移动到画布中的指定点，不创建线条。用 stroke() 方法来画线条。
    ctx.lineTo(b[0], b[1]); //增加一个新点，然后创建一条从上次指定点到目标点的线。Tip: 用 stroke() 方法来画线条
  ...
  isInGraph(x, y) {
    ...
    if (DEBUG_MODE) {
      ...
      this.ctx.strokeRect(transformDelX, transformDelY, delW, delH); //画一个矩形(非填充)
...
export default {
  ...
  paint() {
	...
	// 如果是选中状态，绘制选择虚线框，和缩放图标、删除图标
    if (this.selected) {
      this.ctx.setLineDash([2, 5]); //设置线条宽度。一组描述交替绘制线段和间距（坐标空间单位）长度的数字
      this.ctx.setLineWidth(2); //设置线条的宽度。
      this.ctx.setStrokeStyle(STROKE_COLOR); //设置边框颜色
  ...
  isInGraph(x, y) {
    ...
    if (DEBUG_MODE) {
      // 标识删除按钮区域
      this.ctx.setLineWidth(1); //设置线条的宽度。
      this.ctx.setStrokeStyle('red');
      this.ctx.strokeRect(transformDelX, transformDelY, delW, delH);  //画一个矩形(非填充)
  ...
  //在挂载阶段创建画布
  beforeMount() {
    const sysInfo = uni.getSystemInfoSync(); //获取系统信息同步接口。
  ...
  methods: {
    ...
    start(e) {
      ...
      this.drawArr && this.drawArr.forEach((item, index) => {
        const action = item.isInGraph(x, y);  //即isInGraph函数中有返回三个状态，旋转，移动，还是删除，或者什么也不干

        if (action) {
          item.action = action; //为item添加action属性
    ...
    exportJson() {
      return new Promise((resolve, reject) => {
        ...
        if (this.bgImage) {
          ...
          exportArr.unshift(tmp_img_config); //像数组开头添加元素
